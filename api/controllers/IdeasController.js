    ...
    findCollaborators:function(req,res){
        Ideas.findOne({_id:req.body.id},function(err,data){
            if(err){
                ...
                Student.find({student_id:{$ne:req.decoded.id}},function(error,students){
                    if(error){
                        res.send({success:false,message:'No students'});
                    }else{
                        var intersections = [];
                        students.forEach(element => {
                            if((tags.intersection(new Set(element.tags))).size>0&&!element.requested_collaboration.includes(req.body.id)&&!element.collaborating_on.includes(req.body.id)){
                                intersections.push(element);
                            }
                        });
                        intersections.sort(compareTags);
                        if(intersections.length>0)
                            res.send({success:true,message:'Students found.',students:intersections});
                        else
                            res.send({success:false,message:"No collaborators with given tags."});
                    }
                });
            }
        });
    ...
    requestCollaboration:function(req,res){
        ...
        Student.findOne({student_id:collaboratorId},function(err,data){
            if(err){
                ...
                Student.updateOne({student_id:collaboratorId},{requested_collaboration:requested_collaboration},function(err,data){
                    if(err){
                        res.send({success:false,message:'Unable to send collaboration request.'});
                    }else{
                        res.send({success:true,message:'Collaborator request sent.'});
                    }
                });
            }
        });
